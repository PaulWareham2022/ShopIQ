...
export class OfferRepository extends BaseRepository<Offer> {
  ...
  protected mapRowToEntity(row: any): Offer {
    return {
      ...
      free_shipping_threshold_at_capture: row.free_shipping_threshold_at_capture || undefined,
  ...
  protected mapEntityToRow(entity: Partial<Offer>): Record<string, any> {
    return {
      ...
      free_shipping_threshold_at_capture: entity.free_shipping_threshold_at_capture,
      shipping_included: entity.shipping_included ? 1 : 0,
      amount: entity.amount,
      amount_unit: entity.amount_unit,
      amount_canonical: entity.amount_canonical,
      price_per_canonical_excl_shipping: entity.price_per_canonical_excl_shipping,
      price_per_canonical_incl_shipping: entity.price_per_canonical_incl_shipping,
   ...
   */
  private async getInventoryItemDimension(inventoryItemId: string): Promise<CanonicalDimension> {
    try {
      const sql = `
      ...
      if (result.rows.length === 0) {
        throw new ValidationError(`Inventory item with ID ${inventoryItemId} not found`);
      }
      
      return result.rows.item(0).canonical_dimension as CanonicalDimension;
    } catch (error) {
      throw new DatabaseError('Failed to get inventory item dimension', error as Error);
    }
  }
   ...
   */
  private computeOfferMetrics(input: OfferInput, dimension: CanonicalDimension): {
    ...
    const validation = validateAndConvert(input.amount, input.amount_unit, dimension);
    ...
    const totalWithShipping = shippingIncluded ? input.total_price : input.total_price + shippingCost;
  ...
  async createOffer(input: OfferInput): Promise<Offer> {
    try {
      ...
      const dimension = await getInventoryItemDimension(input.inventory_item_id);
      ...
      const entityData: Omit<Offer, 'id' | 'created_at' | 'updated_at'> = {
        ...
        price_per_canonical_excl_shipping: metrics.pricePerCanonicalExclShipping,
        price_per_canonical_incl_shipping: metrics.pricePerCanonicalInclShipping,
      ...
      throw new DatabaseError('Failed to create offer with unit conversion', error as Error);
    }
  }
   ...
   */
  async findByInventoryItemSortedByPrice(inventoryItemId: string, ascending: boolean = true): Promise<Offer[]> {
    try {
      ...
      throw new DatabaseError('Failed to find offers by inventory item', error as Error);
    }
  }
  ...
  async findBestOfferForItem(inventoryItemId: string): Promise<Offer | null> {
    try {
      const offers = await this.findByInventoryItemSortedByPrice(inventoryItemId, true);
      return offers.length > 0 ? offers[0] : null;
    } catch (error) {
      throw new DatabaseError('Failed to find best offer for item', error as Error);
    }
  }
   ...
   */
  async updateWithRecomputation(id: string, updates: Partial<OfferInput>): Promise<Offer> {
    try {
      // Get current offer
      ...
      const updatedInput: OfferInput = {
        inventory_item_id: updates.inventory_item_id ?? currentOffer.inventory_item_id,
        supplier_id: updates.supplier_id ?? currentOffer.supplier_id,
        supplier_name_snapshot: updates.supplier_name_snapshot ?? currentOffer.supplier_name_snapshot,
        ...
        is_tax_included: updates.is_tax_included ?? currentOffer.is_tax_included,
        tax_rate: updates.tax_rate ?? currentOffer.tax_rate,
        shipping_cost: updates.shipping_cost ?? currentOffer.shipping_cost,
        min_order_amount: updates.min_order_amount ?? currentOffer.min_order_amount,
        free_shipping_threshold_at_capture: updates.free_shipping_threshold_at_capture ?? currentOffer.free_shipping_threshold_at_capture,
        shipping_included: updates.shipping_included ?? currentOffer.shipping_included,
      ...
      if (needsRecomputation) {
        ...
        const dimension = await getInventoryItemDimension(updatedInput.inventory_item_id);
        ...
        if (!updated) {
          throw new DatabaseError(`Failed to update offer with ID ${id}`, new Error('Update returned null'));
        }
        return updated;
      } else {
      ...
      throw new DatabaseError('Failed to update offer with recomputation', error as Error);
    }
  }
}
 ...
 */
async function getInventoryItemDimension(inventoryItemId: string): Promise<CanonicalDimension> {
  try {
    const sql = `
      SELECT canonical_dimension 
      FROM inventory_items 
