...
/**
 * Product variant entity validation schema
 */
export const ProductVariantSchema = BaseEntitySchema.extend({
  /** Foreign key to inventory item */
  inventoryItemId: z.string().uuid('Inventory item ID must be a valid UUID'),
  ...
  barcodeValue: z
    ...
    .regex(/^[0-9]+$/, 'Barcode must contain only digits')
    .nullable()
    .optional(),
 ...
 * Used when creating a brand new variant where id/created_at/updated_at are generated by repository
 */
...
>;
export type ValidatedProductVariant = z.infer<typeof ProductVariantSchema>;
export type ValidatedNewProductVariant = z.infer<
